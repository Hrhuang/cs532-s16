...
readfile = open('countMemento.txt', 'r')
...
def cd(url, backlinksFlag = False):
    ...
    #savefile.write(lowest)

    return lowest

nowdaate = datetime.datetime.now()
...
for line in readfile:
    ...
    if returndate != "":
        date = datetime.datetime.strptime(returndate, '%Y-%m-%dT%H:%M:%S')
        dayys = (nowdaate - date).days

        print dayys
        print type(dayys)
        print link[0]
        print type(link[0])
        output = "{}    {}".format(link[0], dayys)
    ...
    #cd(sys.argv[1])
#elif len(sys.argv) == 3:
    #datimetime.strptime("1995-01-01T12:00:00", '%Y-%m-%dT%H:%M:%S')
