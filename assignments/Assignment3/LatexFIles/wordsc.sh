...
do
	...
	filewords=`grep -c "gamer" "/home/hhuang/Documents/CS532/WebScienceAssig/HW3/Problem1/ProcessedHTMLs/processedHTMLs/"$file`
	link=`grep -w $file "/home/hhuang/Documents/CS532/WebScienceAssig/HW3/Problem2/fileNumWithLink.txt" | awk '{ print $2}'`

	if [ $filewords -ne 0 ] && [ $count -lt 20 ]; 
		then
		count=$[$count+1]
		TF=`awk "BEGIN {printf \"%6.6f\n\", ($filewords/$totalwords)}"`
		TFIDF=`awk "BEGIN {printf \"%6.6f\n\", ($TF*$IDF)}"`
		echo $filewords $totalwords $link >> data1.txt
		echo $TFIDF $TF $IDF $link >> data2.txt
